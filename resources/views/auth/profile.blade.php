...
@section('content')
<style>
    /* 🌿 Enhanced Green Theme - Consistent with Orders Page */
    :root {
    ...
    /* Primary button - consistent with orders page */
    .btn-primary {
        background: linear-gradient(135deg, #2C8F0C, #4CAF50);
        color: white;
...
<div class="container-fluid px-xxl-5 py-4">
    <div class="profile-container">
        ...
        <form method="POST" action="{{ route('profile.update') }}" class="form-section" id="profileForm">
            @csrf
            @method('PUT')
        ...
        <form method="POST" action="{{ route('profile.password') }}" class="form-section" id="passwordForm">
            @csrf
            @method('PUT')
