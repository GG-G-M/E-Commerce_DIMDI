...
class StockInController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Create stock-in and automatically set remaining_quantity
        StockIn::create(array_merge(
            $request->only(['warehouse_id', 'product_id', 'product_variant_id', 'quantity', 'reason']),
            ['remaining_quantity' => $request->quantity]
        ));

        // No manual increment needed: the StockIn model booted() handles it
    ...
    public function update(Request $request, StockIn $stockIn)
    {
        ...
        // Update remaining_quantity safely (cannot go below 0)
